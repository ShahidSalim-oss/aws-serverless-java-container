...
NginxConf.write_https_conf = function(domain){
    ...
    console.log('writedone ', nginx_auto_conf_path+file_name)
}
 ...
 * @returns 
 */
NginxConf.acme_cert = function(domain){
    return new Promise((reslove,reject) => {

        var cmd = `acme.sh --issue -d ${domain} --webroot ${http_web_dir + domain}`
        console.log('\x1B[36m%s\x1B[0m','申请证书命令:',cmd);
        exec(cmd, function (err, stdout, stderr) {
            console.log(err, stdout, stderr);
            reslove();
        })
...
NginxConf.create_new_https_cert = async function (domain) {
    ...
    //申请cert证书
    await NginxConf.acme_cert(domain);

    //安装证书
    await NginxConf.install_cert(domain);
